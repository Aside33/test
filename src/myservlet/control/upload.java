...
public class upload extends HttpServlet {   
	...
	public void uploadFileMethod(HttpServletRequest request,HttpServletResponse response, String logname) 
		...
		int position=secondLine.lastIndexOf('\\'); //截取文件名： 
            ...
            fileName=fileName.replaceAll(" ","");             //文件是否由字母或数字组成判断名字             
            ...
            int j=1;            //确定出文件f1中包含客户上传的文件的内容的最后位置，即倒数第6行:  
            ...
            random2.close();           
            random.close();           
            String uri="jdbc:mysql://localhost:3306/tingzi?useUnicode=true&characterEncoding=utf-8";           Connection con=DriverManager.getConnection(uri,"root","123456");           Statement sql=con.createStatement();           ResultSet rs=sql.executeQuery("SELECT * FROM member where logname = '"+logname+"'");            if(rs.next())           {  if(isLetterOrDigit)              { int mm=sql.executeUpdate("UPDATE member SET pic= '"+savedFileName+"' where logname = '"+logname+"'");                if(mm!=0)                { backNews=fileName+"成功上传";                  upFile.setFileName(fileName);                  upFile.setSavedFileName(savedFileName);                  upFile.setBackNews(backNews);                }              }            }           con.close();            f1.delete();          }      catch(Exception exp)         {  backNews=""+exp;            upFile.setBackNews(backNews);         }      try{  RequestDispatcher dispatcher=  request.getRequestDispatcher("showUploadMess.jsp");//转发 
